# Отчет по лабораторной работе №1

## 1. Выделение компонент R, G, B
Были разделены каналы изображения. Ниже представлены результаты:
- Канал R: ![](img/Output_R.png)
- Канал G: ![](img/Output_G.png)
- Канал B: ![](img/Output_B.png)

## 2. Преобразование изображения в HSI
Ниже представлены компоненты:
- Яркостная компонента: ![](img/Output_Intensity.png)
- Инвертированная яркостная компонента: ![](img/Output_Inverted_Intensity.png)

## 3. Изменение размера изображения

### 3.1 Изначальные размеры изображения
Размер изображения: (1024, 1024)

### 3.2 Растяжение (интерполяция)
Коэффициент растяжения: M = 3  
Результат растяжения: ![](img/Output_Stretched.png)  
Размер изображения: (3072, 3072)

### 3.3 Сжатие (децимация)
Коэффициент сжатия: N = 2  
Результат сжатия: ![](img/Output_Compressed.png)  
Размер изображения: (512, 512)

### 3.4 Передискретизация в два прохода (растяжение + сжатие)
Коэффициент: K = M / N = 1.5  
Результат передискретизации в два прохода: ![](img/Output_TwoPass_Resampled.png)  
Размер изображения: (1536, 1536)

### 3.5 Передискретизация в один проход
Коэффициент: K = 1.5  
Результат передискретизации в один проход: ![](img/Output_OnePass_Resampled.png)  
Размер изображения: (682, 682)


# Лабораторная работа Nº2: Обесцвечивание и бинаризация растровых изображений

## Исходные изображения: 

![](img/book.png)

![](img/fingers.png)

![](img/cat_2.png)

## Этап 1: Приведение полноцветного изображения к полутоновому
![Полутоновое изображение](img/grayscale_1.bmp)

![Полутоновое изображение](img/grayscale_2.bmp)

![Полутоновое изображение](img/grayscale_3.bmp)

  
Формула вычисления яркости каждого пикселя:
  Y = 0.3 · R + 0.59 · G + 0.11 · B

## Этап 2: Приведение полутонового изображения к монохромному методом пороговой обработки
- **Метод:** Адаптивная бинаризация WAN
- **Размер окна:** 5×5
- Порог вычисляется по формуле:
T = m_max * (1 - k * (1 - s/R))
m_max = (max + m) / 2
![Бинарное изображение](img/binary_1.bmp)
![Бинарное изображение](img/binary_2.bmp)
![Бинарное изображение](img/binary_3.bmp)

# Лабораторная работа No3
## Фильтрация изображений и морфологические операции

**Метод:** Ранговый фильтр с маской прямой крест (окно 3x3, ранг 4)

### Этапы обработки:
1. Исходные изображения
![](img/image_3.png)

![](img/image_2.png)

2. Применение рангового фильтра к изображению.
![](img/filtered_1.png)

![](img/filtered_2.png)

3. Вычисление разностного изображения как абсолютной разности между исходным и отфильтрованным изображениями.
![](img/difference_1.png)
![](img/difference_2.png)

# Лабораторная работа No4: Выделение контуров на изображении

**Вариант:** Оператор Приюитт 5x5

## 1. Исходное цветное изображение
![Исходное изображение](img/fox1_img.png)

## 2. Полутоновое изображение
![Полутоновое изображение](img/grayscale.png)

## 3. Градиентные матрицы

### 3.1. Горизонтальная градиентная матрица Gx (нормализована)
![Градиент Gx](img/Gx.png)

### 3.2. Вертикальная градиентная матрица Gy (нормализована)
![Градиент Gy](img/Gy.png)

### 3.3. Итоговая градиентная матрица G = sqrt(Gx^2 + Gy^2) (нормализована)
![Градиент G](img/G.png)

## 4. Бинаризованная градиентная матрица G
*Порог бинаризации: 25 (значения выше порога принимают значение 255, остальные 0)*
![Бинаризованное изображение G](img/G_binary.png)


# Лабораторная работа №5. Выделение признаков символов

## Символ 

![Изображение](img/symbol_0.png)

### Профили:
![Профиль X](img/symbol_0_x_profile.png)
![Профиль Y](img/symbol_0_y_profile.png)

### Признаки:
- Вес каждой четверти: [150, 1, 0, 0]
- Удельный вес: [0.05547337278106509, 0.0003698224852071006, 0.0, 0.0]
- Центр тяжести: (30.78, 36.85)
- Нормированные координаты центра тяжести: (0.2960, 0.3543)
- Моменты инерции: (X: 495.50, Y: 14069.79)
- Нормированные моменты инерции: (X: 0.0458, Y: 1.3008)

## Символ 

![Изображение](img/symbol_1.png)

### Профили:
![Профиль X](img/symbol_1_x_profile.png)
![Профиль Y](img/symbol_1_y_profile.png)

### Признаки:
- Вес каждой четверти: [346, 1, 0, 0]
- Удельный вес: [0.1279585798816568, 0.0003698224852071006, 0.0, 0.0]
- Центр тяжести: (30.53, 27.03)
- Нормированные координаты центра тяжести: (0.2936, 0.2599)
- Моменты инерции: (X: 33311.59, Y: 23708.43)
- Нормированные моменты инерции: (X: 3.0798, Y: 2.1920)

## Символ 

![Изображение](img/symbol_2.png)

### Профили:
![Профиль X](img/symbol_2_x_profile.png)
![Профиль Y](img/symbol_2_y_profile.png)

### Признаки:
- Вес каждой четверти: [149, 0, 0, 0]
- Удельный вес: [0.055103550295857985, 0.0, 0.0, 0.0]
- Центр тяжести: (20.01, 24.25)
- Нормированные координаты центра тяжести: (0.1924, 0.2332)
- Моменты инерции: (X: 14531.81, Y: 420.99)
- Нормированные моменты инерции: (X: 1.3435, Y: 0.0389)

## Символ 

![Изображение](img/symbol_3.png)

### Профили:
![Профиль X](img/symbol_3_x_profile.png)
![Профиль Y](img/symbol_3_y_profile.png)

### Признаки:
- Вес каждой четверти: [174, 0, 2, 0]
- Удельный вес: [0.0643491124260355, 0.0, 0.0007396449704142012, 0.0]
- Центр тяжести: (20.37, 30.31)
- Нормированные координаты центра тяжести: (0.1959, 0.2915)
- Моменты инерции: (X: 25053.81, Y: 514.99)
- Нормированные моменты инерции: (X: 2.3164, Y: 0.0476)

## Символ 

![Изображение](img/symbol_4.png)

### Профили:
![Профиль X](img/symbol_4_x_profile.png)
![Профиль Y](img/symbol_4_y_profile.png)

### Признаки:
- Вес каждой четверти: [448, 84, 0, 0]
- Удельный вес: [0.16568047337278108, 0.03106508875739645, 0.0, 0.0]
- Центр тяжести: (37.56, 27.13)
- Нормированные координаты центра тяжести: (0.3612, 0.2608)
- Моменты инерции: (X: 50364.56, Y: 85009.08)
- Нормированные моменты инерции: (X: 4.6565, Y: 7.8596)

## Символ 

![Изображение](img/symbol_5.png)

### Профили:
![Профиль X](img/symbol_5_x_profile.png)
![Профиль Y](img/symbol_5_y_profile.png)

### Признаки:
- Вес каждой четверти: [447, 10, 0, 0]
- Удельный вес: [0.16531065088757396, 0.0036982248520710057, 0.0, 0.0]
- Центр тяжести: (28.22, 32.39)
- Нормированные координаты центра тяжести: (0.2714, 0.3114)
- Моменты инерции: (X: 57302.67, Y: 44783.23)
- Нормированные моменты инерции: (X: 5.2980, Y: 4.1405)

## Символ 

![Изображение](img/symbol_6.png)

### Профили:
![Профиль X](img/symbol_6_x_profile.png)
![Профиль Y](img/symbol_6_y_profile.png)

### Признаки:
- Вес каждой четверти: [247, 86, 0, 0]
- Удельный вес: [0.09134615384615384, 0.031804733727810654, 0.0, 0.0]
- Центр тяжести: (36.28, 26.73)
- Нормированные координаты центра тяжести: (0.3488, 0.2570)
- Моменты инерции: (X: 8503.21, Y: 89269.03)
- Нормированные моменты инерции: (X: 0.7862, Y: 8.2534)

## Символ 

![Изображение](img/symbol_7.png)

### Профили:
![Профиль X](img/symbol_7_x_profile.png)
![Профиль Y](img/symbol_7_y_profile.png)

### Признаки:
- Вес каждой четверти: [180, 0, 2, 0]
- Удельный вес: [0.06656804733727811, 0.0, 0.0007396449704142012, 0.0]
- Центр тяжести: (20.80, 27.89)
- Нормированные координаты центра тяжести: (0.2000, 0.2682)
- Моменты инерции: (X: 20257.80, Y: 562.88)
- Нормированные моменты инерции: (X: 1.8729, Y: 0.0520)


# Лабораторная работа №6: Сегментация текста

## 1. Исходное изображение фразы
![Фраза](img/phrase.bmp)

## 2. Профили изображения
![Профили](img/profiles.png)

## 3. Результат сегментации
![Сегментация](img/segmented.png)

Координаты символов:
- Символ 1: (4, 58, 112, 89)
- Символ 2: (140, 1, 248, 89)
- Символ 3: (276, 1, 305, 119)
- Символ 4: (372, 1, 402, 119)
- Символ 5: (429, 1, 577, 87)
- Символ 6: (605, 9, 723, 111)

## 4. Профили символов алфавита
### Символ 𐎀 (индекс 0)
![Профили символа 0](img/profile_0.png)

### Символ 𐎁 (индекс 1)
![Профили символа 1](img/profile_1.png)

### Символ 𐎂 (индекс 2)
![Профили символа 2](img/profile_2.png)

### Символ 𐎃 (индекс 3)
![Профили символа 3](img/profile_3.png)

### Символ 𐎄 (индекс 4)
![Профили символа 4](img/profile_4.png)

### Символ 𐎅 (индекс 5)
![Профили символа 5](img/profile_5.png)

### Символ 𐎆 (индекс 6)
![Профили символа 6](img/profile_6.png)

### Символ 𐎇 (индекс 7)
![Профили символа 7](img/profile_7.png)

### Символ 𐎈 (индекс 8)
![Профили символа 8](img/profile_8.png)

### Символ 𐎉 (индекс 9)
![Профили символа 9](img/profile_9.png)


# Лабораторная работа №7: Классификация символов

## Исходная строка: `𐎀𐎁𐎂𐎃𐎄𐎅𐎆`
## Распознанная строка: `𐎀𐎁𐎂𐎃𐎄𐎅𐎆`
## Точность: 100.00%

## Подробные гипотезы:
1: [('𐎀', 1.0), ('𐎐', 0.988862106830164), ('𐎚', 0.8317248376087067), ('𐎙', 0.8189746407689054), ('𐎔', 0.7582485729545444)]
2: [('𐎁', 1.0), ('𐎄', 0.971064326069508), ('𐎛', 0.957870449524817), ('𐎜', 0.9559295180396093), ('𐎗', 0.9533868280317249)]
3: [('𐎂', 1.0), ('𐎕', 0.9786334630580517), ('𐎒', 0.9744199615740724), ('𐎇', 0.9668913157040963), ('𐎌', 0.9463217298894121)]
4: [('𐎃', 1.0), ('𐎏', 0.9739007624298271), ('𐎇', 0.966949947446232), ('𐎓', 0.9631574086494877), ('𐎈', 0.9616653926254867)]
5: [('𐎄', 1.0), ('𐎁', 0.971064326069508), ('𐎜', 0.9702415850639925), ('𐎛', 0.9517782880889245), ('𐎊', 0.9477683829610167)]
6: [('𐎅', 1.0), ('𐎗', 0.9389366646107358), ('𐎋', 0.9279427142400193), ('𐎆', 0.9271539694058465), ('𐎛', 0.9232942168821753)]
7: [('𐎆', 1.0), ('𐎋', 0.9868486766778397), ('𐎗', 0.9783471307765299), ('𐎁', 0.9493486866323515), ('𐎛', 0.948528646712665)]

## Эксперимент с изменённым шрифтом
Размер шрифта: 20
Распознано: 𐎀𐎁𐎂𐎂𐎄𐎅𐎆
Точность: 95.00%

# Лабораторная работа №8. Текстурный анализ и контрастирование

## Результаты

Матрица: NGLDM

### Исходные и преобразованные изображения
![Изображения](img/lab8_images.png)

### Матрицы NGLDM
![NGLDM](img/lab8_ngldm.png)

### Текстурные признаки
- Исходное изображение: SNE = 0.0002, LNE = 21576.3772
- Преобразованное изображение: SNE = 0.0000, LNE = 33691.4737

# Лабораторная работа №9. Анализ шума

## 1. Исходные данные
- Анализируемый файл: `sound.wav`
- Частота дискретизации: 44100 Гц
- Длительность: 2.44 сек
- Уровень шума: 0.0054

## 2. Спектральный анализ

### Оригинальный сигнал
![Спектрограмма](img/original_spectrogram.png)

### Результаты фильтрации
| Фильтр | Спектрограмма |
|--------|---------------|
| Савицкого-Голея | ![Спектрограмма](img/savgol_spectrogram.png) |
| Винера | ![Спектрограмма](img/wiener_spectrogram.png) |
| Низких частот | ![Спектрограмма](img/lowpass_spectrogram.png) |

## 3. Анализ энергии
- Шаг анализа: 0.1 сек
- Анализируемый частотный диапазон: 40-50 Гц
- Моменты с максимальной энергией: [1.3, 0.5, 1.8] сек

![График энергии](img/energy_analysis.png)


# Лабораторная работа №10. Обработка голоса

## Звук "А"
- Минимальная частота: 193.80 Гц
- Максимальная частота: 904.39 Гц
- Основной тон с наибольшим количеством обертонов: 215.33 Гц
- Три самые сильные форманты: 215.33, 430.66, 861.33 Гц

![Спектрограмма](img/А_spectrogram.png)

## Звук "И"
- Минимальная частота: 172.27 Гц
- Максимальная частота: 430.66 Гц
- Основной тон с наибольшим количеством обертонов: 215.33 Гц
- Три самые сильные форманты: 193.80, 366.06, 559.86 Гц

![Спектрограмма](img/И_spectrogram.png)

## Звук "мяуканье"
- Минимальная частота: 193.80 Гц
- Максимальная частота: 925.93 Гц
- Основной тон с наибольшим количеством обертонов: 236.87 Гц
- Три самые сильные форманты: 236.87, 473.73, 689.06 Гц

![Спектрограмма](img/мяуканье_spectrogram.png)


